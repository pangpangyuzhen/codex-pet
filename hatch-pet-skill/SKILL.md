---
name: hatch-pet
description: Create, repair, validate, visually QA, install, and package Codex-compatible animated pets and pet spritesheets from character art, generated images, company or prospect brand cues, or visual references. Use for standard or multi-form state-aware pets, non-pixel custom styles, mascot pets, full 8x9 atlases, cache-safe revisions, wake/install troubleshooting, transparent-cell QA, contact sheets, and pet.json packaging. This skill composes the installed $imagegen system skill for visual generation and uses bundled scripts for deterministic spritesheet assembly.
---
...
Choose one mode before preparing prompts:

- **Standard mode:** preserve one canonical form across all rows and vary pose, expression, and motion.
- **Multi-form mode:** use when the user asks for transformations, work forms, thinking forms, success forms, costumes, powered-up states, or state-specific props. Read [references/multi-form-workflow.md](references/multi-form-workflow.md) before prompt preparation, assembly, installation, or repair.

For any installed-pet repair, cache issue, missing first wake, wrong visible form, or “Create your own pet” failure, also read [references/install-and-runtime-diagnostics.md](references/install-and-runtime-diagnostics.md).
...
For multi-form runs, apply the state mapping and designed-form priority rules from `references/multi-form-workflow.md` before generating any row. Do not generate a generic default form first and later bolt transformations onto it; define the form matrix and canonical invariants up front.
